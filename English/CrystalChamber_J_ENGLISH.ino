...
#define PVentilador 10  //Internal fan
#define Pvolt A0		//Pin for voltage divider reading
#define BT_up A1        //Button 1
#define BT_mid A2       //Button 2
#define Ptermo A3  		//Thermostat servo
...
unsigned char visor = 0;    // Variable to track the display status (0 - off) (1 - operation) (2 - mode selection menu)
...
bool HB = false; 	 // Variable to signal HeatBooster algorithm activation
...
int varVE = 9000;      // Variation of the external temperature moving average in the last hour
int varVU = 9000;      // Variation of the moving average of humidity in the last hour
int adcRead = 0;           // ADC reading
int Vtec = 0;              // Average of TEC voltage supply
int Valvo = 300;           // TEC voltage target, initially at 3 volts
unsigned int termSec = 0;  // Thermostat servo operation time
...
int conE[3];  // External temperature tracking array (0: Record from 15 minutes ago) (1: 5-reading moving average) (2: External temperature, reference for HeatBooster)
...
unsigned long boostLT = 0;                    // Last timer Booster was used 
...
// Global vars for thermal control
int DTc;  // Delta Tc, variable to compensate for DT
float DT = 0.0;  // Temperature difference between external and target
...
void setup() {
  ...
  dbgServo(); // Visual check-up
  opServo(9);  // Close the desiccant trapdoor
	
  //Thermostat initialization
  termoServo.attach(Ptermo);  //Thermostart continuous servo
  termoServo.write(90, 255);  //Stop position

  delay(1000);   //Wait a bit for first reading
  TempReg();     //First reading
  delay(1000);   //conformation
  Termostato();  //First thermostat call
}
  ...
  if (OPE == 0) {
    ...
    if (OPT == true) {
      if (RPMLT == 0) {    //If fan is always working ok...
        if (rpm < 1900) {  // Check for failure in rotation (my fan usual RPM reading is between 2000 and 2100 RPM, so anything below 1900 is probably a problem)
          RPMLT = millis();
        }
      } else { //The fan failed...
        if ((millis() - RPMLT) > 60000UL) {  // ...more than 60 seconds ago?
          if (rpm > 2000) {                  // Check for a false-positive, if it's back in the range, no problem 
            RPMLT = 0;
	...
	if ((OPE >= 1) && (OPE <= 3)) {  //For thermal control operational modes
...
void leTMP() {
  ...
  if ((millis() - varLT) >= varInterval) {
	...
	//Updates external temperature history
	if (conE[1] > conE[0]) {
		varM = 7000;
	...
	//HeatBooster - Turn off verification
    if (HB == true) {  //HeatBooster is on, check external temperature stability
      calc = (int)(varVE / 100);
      switch (calc) {  //ignore cases 17 and 19
        case 11:
        case 12:
        case 21:
        case 22:
          HB = false;   //turns HeatBooster off
          conE[2] = 0;  //zeroes the reference
          break;
      }
    }
  ...
  if ((millis() - termLT) >= termInterval) {  //Check for interval
	...
	//HeatBooster Algorithm - Booster in cooling power to antecipate an external temperature warming
    //Trigger : rising the mean external temperature
    if (conE[1] > conE[0]) {  //The mean is higher then the register
      if (conE[2] == 0) {     //There's no reference point
        conE[2] = conE[1];    //Registers the mean temperature at the time
        boostLT = millis();   //Registers the time
      }
    } else if (conE[1] == conE[0]) {  //If the variation is constant
      if (conE[2] != 0) {             //And there's a previous register 
        boostLT = millis();           //Updates the time to cover the occurrence of a slow temperature rise
      }
    } else {        //Mean is falling
      conE[2] = 0;  //Reset
      boostLT = 0;  //Reset
    }

    //Verify the booster
    if ((OPE >= 1) && (OPE <= 3)) {  //In thermal control modes
      if ((conE[2] != 0) && (HB == false)) {//There's a register and the booster is off
        if ((millis() - boostLT) > 6000) { //Wait for 6 seconds to avoid division by zero
          if (iArr((100 * (conE[1] - conE[2])) / ((millis() - boostLT) / 6000)) > 4) {  //Verify if heating rate is higher then [0,245°C / 60 min] (!) Check your usual heating rate to keep the algorithm adequate to your conditions
            DTc = DTc + 6; //Increase DTc in 0.6 °C (!) Experimentally determined, it is advisable to experiment to correctly adjust it to your physical arrangement
            HB = true;
            // Restart mean external temperature observation
            conE[0] = conE[1];
            varVE = 9000;
          }
        }
...
void TempReg() {
  ...
  //Verify TEC voltage
  if (OPT == 1) {  //TEC is on
	  Vtec = tensaoTEC(); //Voltage reading
  } else {
    Vtec = 0;  //Registers as zero because TEC is off
  }
  ...
  if (visor == 1) {
	...
	//Thermostat
    display.print(Vtec / 100.0, 1);  //Actual voltage in decimals
    display.print(" ");
    display.println(DTc);  //DTc value
  ...
  if (SDticks == 0) { //SD information
    myFile.print(DTc);  //DTc value
	quebra();
	myFile.print(US[1]);  //humidity servo position
    quebra();
    myFile.print(Vtec / 100.0, 2);  //TEC voltage
    quebra();
    myFile.print(HB); //HeatBooster status
...
void DispReset() {
  ...
  if (HB == true) {
    ...
    display.print(F("+"));  //Signals HeatBooster usage
  }
...
void Termostato() {
  ...
  if ((OPE >= 1) && (OPE <= 3)) {
	...
	DT = 0.0;     // Resets global variable for DT

    if (temperature[2] != 0) {//Bypass in case the DHT is faulty
      //calculates the deviation from target
      if (conE[1] == 0) { //If there is no average yet
        DT = (temperature[2] - Talvo) / 10.0;  
      } else {
        DT = (conE[1] - Talvo + DTc) / 10.0;  
      }

      DTfun = (2.49 * DT * DT) + (32.45 * DT) + (118.48);  //DT to Voltage (dV)
      
	  // Domain restriction (!) Adjust accordingly to your voltage regulator
      if (DTfun < 160) { //minimum of 1.6 volts
        DTfun = 160;
      } else if (DTfun >= 1120) { //maximum of 11.2 volts
        DTfun = 1120;
      }
      Valvo = (int)DTfun;  //Trucated as integer
	  
	  //Checks if adjust is needed
      if (OPT == true) {  //Skips processing if TEC is off
        //Registers temperature data in equilibrium for calibration curve refining
        if ((int)(varV / 10) == 111) {  //3 turns of constant temperature
          if (vSD == true) {            //confirms SD presence
            myFile.print(millis());
            quebra();
            myFile.print("A");  //Tag for identification (useful for Excel filters)
            quebra();
            myFile.print(conE[1] / 10.0, 1);  //Mean external temperature
            quebra();
            myFile.print(conT[2] / 10.0, 1);  //Mean temperature
            quebra();
            myFile.print(Vtec / 100.0, 2);  //voltage reading
            myFile.println("");             
            myFile.flush();                 
          }
        }

		//Checks tolerance for adjust (!) Varies accordingly to the physical arrangement, collect data for better fitting to your system		
		//Actual: Amplitude (0.3v) Standard deviation (0.08v) Positive amplitude (0.16) Negative amplitude(0.08)
        if (abs(Vtec - Valvo) >= 16) { //Adjust only if difference is higher then 0.16v 
          //Loop of 4 adjust iterations
          for (int i = 0; i <= 3; i++) {
            ajustaVolt();                   //Call the adjust
            delay(100);                     //Waits for new reading
            Vtec = tensaoTEC();             //Verifies
            if (abs(Vtec - Valvo) <= 10) {  //Target reached, break the loop
              break;
            }
          }
        }
...
void TermoAjuste() { 
  //Function for DT adjustment
  //DT > 0 indicates the need to increase cooling power

  if (HB == true) { //If booster was activated
    calc = (conT[0] - Talvo) / 3; //Reduce the adjustment intensity in 1/3 (avoids that booster gains are undone before making effect)
    if (calc < 0) {  //Minimal adjustment
      calc = min(calc, -1);
    } else if (calc > 0) {
      calc = max(calc, 1);
    } 
  } else {  //Booster is off, default adjustment
    calc = (conT[0] - Talvo);
  }
  DTc = DTc + calc;  //Makes the adjustment
  //Limit conditions
  if (conT[0] < Talvo) {            //DTc reduction
    if ((conE[1] - Talvo) <= 15) {  //Minimum power situation -> Regulator at minimal voltage kept 1.5°C difference
      if (HB == true) {             //if booster was activated
        DTc = max(6, DTc);          //keep at least the booster value
      } else {
        DTc = max(0, DTc);  //null or positive, due to minimal power
      }
    }
  } 

  Termostato();  //Call adjustment
}
int tensaoTEC() {
  //Function that reads TEC voltage
  adcRead = 0;  //reset value
  for (int j = 0; j <= 14; j++) { //15 samples
    adcRead = adcRead + analogRead(Pvolt);  //Raw signal reading from 0 a 1023 in cumulative sum
    delay(100);                             //waits 100 ms between readings
  }
  //Voltage divider calculations:
  //R1 = 20000 R2 = 10000 Voltage Amplitude = 5 v
  //with 15 samples: (15/(1024*15)) = (1/1024) in volts
  adcRead = (int)(adcRead / 10.24);  //Calculations in cV
  return adcRead;                    //Returns
}

void ajustaVolt() {
  //Functions that control the continuous servo termoServo
  //(!) Check the angular speed corresponding to each value of .write() function for a correct calculation  
  //Calculate movement duration
  termSec = (int)(18.18 * (abs(Vtec - Valvo)));  // 1000 [ms] / 55 [cV/s] = 18.18 in [ms]
  //Verify adjustment direction
  if (Vtec > Valvo) {  //Voltage reduction
    termoServo.write(84, 255);  //Start rotation
  } else if (Vtec < Valvo) {    //Voltage increase
    termoServo.write(96, 255);  //Start rotation
  }
  delay(termSec);             //Waits the movement duration
  termoServo.write(90, 255);  //Send stop signal
}
